...
def encode_value(obj, heap):
    if obj['kind'] == 'base':
        ...
        members.sort(key=lambda ee: ee[1]['addr'])
        for k, v in members:
...
void *x = foo(); // <-- there is an extraneous step here AFTER foo returns but
    ...
    '''
    for prev, cur, nextf in zip(final_execution_points, final_execution_points[1:], final_execution_points[2:]):
        if prev['event'] == 'return' and len(prev['stack_to_render']) > 1:
            ...
            # if cur['func_name'] == nextf['func_name'] because otherwise
